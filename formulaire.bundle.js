...
.content .container-2 .formulaire form .options p {
  margin-bottom: 30px;
  width: 95%;
  text-align: left;
}
...
.content .container-2 .formulaire form .submit-container {
  cursor: pointer;
}
...
const createRadioInput = (element, name) => {
  ...
  if (element[3]) {
    divElement3.append(input3, label3);
  }
  const divContainerElement = document.createElement("div");
  divContainerElement.classList.add("radio");
  divContainerElement.append(divElement1, divElement2);
  if (element[3]) {
    divContainerElement.append(divElement3);
  }
  if (element[3]) {
    const pElement = document.createElement("p");
    pElement.textContent = "Laquelle de ces options privilegiez vous dans vos choix ?";
    document.getElementById("form-options").appendChild(pElement);
  }
  document.getElementById("form-options").appendChild(divContainerElement);
};
const createFinalChoiceHTML = () => {};
...
const sendData = async formData => {
  try {
    await fetch("http://164.132.229.216:6601/", {
      ...
      document.getElementById("formulaire-submit-button").textContent = "Envoyer";
    }).catch(reason => {
      console.log(reason);
      createSubmitModal(false);
      document.getElementById("formulaire-submit-button").textContent = "Renvoyer";
    });
  } catch {
...
const verifyValidityForm = () => {
  ...
  document.getElementById("formulaire-submit-button").textContent = "Envoie en cours...";
  return true;
};
...
document.getElementById("formulaire-submit-button").addEventListener("click", async event => {
  ...
  if (document.getElementById("formulaire-submit-button").textContent !== "Envoie en cours...") {
